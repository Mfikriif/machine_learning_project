...
model = joblib.load('kmeans_3.pkl')
# Memuat model KMeans
...
gender = st.sidebar.selectbox("Gender (0 = Female, 1 = Male)", options=[0, 1], index=0)
...
favc = st.sidebar.selectbox("Frequent Consumption of High Calorie Food (FAVC)", options=[0, 1], index=0)
ch2o = st.sidebar.slider("Daily Water Intake (liters)", min_value=0.1, max_value=10.0, step=0.1, value=2.0)
family_history = st.sidebar.selectbox("Family History with Overweight", options=[0, 1], index=0)
